public class BankAccount {
    ...
    public boolean depositMoney(int amount) {
    	// task 1a
    	//program that adding money and updating the balance, and returning if this action was executed
    	boolean ans = false;
        if(amount>=0) { //we will add only positive amounts
        	ans = true;
        	this.balance = this.balance + amount; // updating the balance
        }
        return ans;
    }
    ...
    public boolean withdrawMoney(int amount) {
    	// task 1b
    	//program that taking out money from bank account, and returning if this action was executed
        boolean ans = false;
        //we can take money only if we can effort this by our balance and the amount can't be negative.
