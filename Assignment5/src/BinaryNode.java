public class BinaryNode<T> {
    ...
    //help function to task 4
    private String toStringRecursive(String ans, int depth) {
    	// program that print our tree in 2D
    	if(left != null) // calling the function to place the current left node in the place
    		ans = left.toStringRecursive(ans, depth+1); 
    	
    	for(int i =0;i<depth;i++) { //adding 2*d spaces 
    		ans = ans + "  ";
    	}
    	ans = ans + this.data.toString() + "\n"; //adding the object data and new line
    	
    	if(right != null) // calling the function to place the current right node in the place
    		ans = right.toStringRecursive(ans, depth+1);
